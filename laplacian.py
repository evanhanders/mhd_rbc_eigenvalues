...
def mhd_rbc_evp(Q, Ra, k, Nz=24, variables_form=True):
    ...
    if variables_form:
        variables=['T','Tz','p','u','v','w','phi','Ax','Ay','Az','Bx','By', 'Bz','Jx','Jy','uz','vz','wz']
    else:
        variables=['T','Tz','p','u','v','w','phi','Ax','Ay','Az','Bx','By','uz','vz','wz']
    ...
    problem.add_equation("dt(u)  + dx(p)   + inv_Re_ff*Lap(u, uz) - JxB_pre*Jy     = v*Oz - w*Oy + JxB_pre*(Jy*Bz - Jz*By)")
    problem.add_equation("dt(v)  + dy(p)   + inv_Re_ff*Lap(v, vz) + JxB_pre*Jx     = w*Ox - u*Oz + JxB_pre*(Jz*Bx - Jx*Bz) ")
    problem.add_equation("dt(w)  + dz(p)   + inv_Re_ff*Lap(w, wz)              - T = u*Oy - v*Ox + JxB_pre*(Jx*By - Jy*Bx) ")
