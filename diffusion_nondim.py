...
def mhd_rbc_evp(Q, Ra, k, Nz=24, variables_form=True):
    ...
    if variables_form:
        variables=['T','Tz','p','u','v','w','phi','Ax','Ay','Az','Bx','By', 'Bz','Jx','Jy','Ox','Oy']
    else:
    ...
    problem.substitutions['St'] = 'Pr/Pm'

    #Equations
    problem.add_equation("dt(T) + w*dzT0   - Lap(T, Tz)          = -UdotGrad(T, Tz)")

    problem.add_equation("dt(u)  + dx(p)   + Pr*Kx - Q*Pr*St*Jy     = v*Oz - w*Oy + Q*Pr*St*(Jy*Bz - Jz*By)")
    problem.add_equation("dt(v)  + dy(p)   + Pr*Ky + Q*Pr*St*Jx     = w*Ox - u*Oz + Q*Pr*St*(Jz*Bx - Jx*Bz) ")
    problem.add_equation("dt(w)  + dz(p)   + Pr*Kz - Ra*Pr*T        = u*Oy - v*Ox + Q*Pr*St*(Jx*By - Jy*Bx) ")

    problem.add_equation("dt(Ax) + dx(phi) + St*Jx - v             = v*Bz - w*By")
    problem.add_equation("dt(Ay) + dy(phi) + St*Jy + u             = w*Bx - u*Bz")
    problem.add_equation("dt(Az) + dz(phi) + St*Jz                 = u*By - v*Bx")
...
if __name__ == '__main__':
    ...
    for j in range(3):
        ...
        fig.savefig('vars_vs_subs_nz{}_ra{}_k{}.png'.format(nzs[j], Ra, k), bbox_inches='tight', dpi=300)
        plt.clf()
    ...
    for j in range(len(file_label)):
        ...
        fig.savefig('{}_low_hi_res_ra{}_k{}.png'.format(file_label[j], Ra, k), bbox_inches='tight', dpi=300)
        plt.clf()
