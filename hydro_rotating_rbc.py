...
def rotating_rbc_evp(Ek, Ra, k, Nz=24):
    ...
    problem.add_equation("dt(u)  + dx(p)   + inv_Re_ff*(Kx - v/E)     = v*Oz - w*Oy")
    problem.add_equation("dt(v)  + dy(p)   + inv_Re_ff*(Ky + u/E)     = w*Ox - u*Oz")
...
if __name__ == '__main__':
    ...
    problems = []
    for nz in nzs:
        problems.append(rotating_rbc_evp(Ek, Ra, k, Nz=nz))
    ...
    for j in range(3):
        ...
        fig.savefig('rotating_hydro_nz{}_ra{}_k{}.png'.format(nzs[j], Ra, k), bbox_inches='tight', dpi=300)
        plt.clf()
    ...
    fig.savefig('rotating_hydro_low_hi_res_ra{}_k{}.png'.format(Ra, k), bbox_inches='tight', dpi=300)
    plt.clf()
